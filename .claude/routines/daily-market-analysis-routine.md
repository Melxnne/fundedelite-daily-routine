...
6. Erstelle `reports/report_<YYYY-MM-DD>.md` mit:
   - Gold (GC=F) und US500 (ES=F): aktuelle Session-Highs/Lows (Asia/London/NY,
     ...
     Highs-Lows auf dem 5m-Chart, HTF-Bias aus den 1h-Candles (bullish/bearish/range,
     anhand Struktur - Higher-Highs/Lows vs. Lower-Highs/Lows auf 1h).
   ...
   - **Setup-Sektion (nur falls Schwelle erreicht):** Wenn für Gold oder US500
     mindestens 5 von 8 Confluence-Faktoren erfüllt sind UND der News-Filter nicht
     greift, explizit benennen:
     - Symbol
     - Richtung (long/short) - muss mit dem HTF-Bias übereinstimmen, sonst als
       Konflikt markieren statt Setup vorzuschlagen
     - HTF-Bias, auf den das Setup abzielt (z.B. "folgt 1h-Aufwärtsstruktur,
       Zielzone vorheriges 1h-High")
     - Welche Faktoren erfüllt sind, welche nicht
     - Relevante Levels (Entry-Zone, FVG/OB-Grenzen)
     Wenn die Schwelle nicht erreicht wird: kurz "kein Setup nach Rulebook" + welche
     Faktoren fehlen.
...
7. Sei ehrlich und direkt, keine Beschönigung (siehe Trading-Honesty-Vorgabe). Das
   ...
   Report auch so benennen, nicht so tun als wäre es dein Live-Account. Die
   Setup-Sektion ist eine Regelanwendung, keine Empfehlung - wenn Faktoren knapp
   oder zweideutig erfüllt sind, das explizit so benennen statt es glattzubügeln.
